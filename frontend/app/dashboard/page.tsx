    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-32 rounded-xl bg-white animate-pulse border border-zinc-200" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-1">Here's what's happening in your committee today.</p>
        </div>
      </div>
 
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {statCards.map((stat, index) => (
          <Card key={index} className="bg-white border-zinc-200 shadow-sm">
