  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Notifications</h1>
          <p className="text-zinc-400 mt-1">Stay updated with activities across ECOS</p>
        </div>
        {unreadExist && (
          <Button 
            variant="outline" 
      ...
      <div className="space-y-3">
        {notifications.map((n) => (
          <Card 
            ...
            className={`transition-colors ${
              n.isRead ? 'bg-zinc-950/50 border-zinc-800' : 'bg-zinc-900/80 border-indigo-500/30 shadow-[0_0_15px_rgba(79,70,229,0.05)]'
            }`}
          >
            <CardContent className="p-4 flex gap-4">
              <div className={`mt-0.5 rounded-full p-2 h-fit ${n.isRead ? 'bg-zinc-800 text-zinc-500' : 'bg-indigo-500/20 text-indigo-400'}`}>
                {n.type === 'SYSTEM' ? <AlertCircle className="w-5 h-5" /> : <Bell className="w-5 h-5" />}
              </div>
              <div className="flex-1 space-y-1">
                <div className="flex justify-between items-start">
                  <p className={`text-sm ${n.isRead ? 'text-zinc-400' : 'text-zinc-200 font-medium'}`}>
                    {n.message}
                  </p>
        ...
        {notifications.length === 0 && (
          <div className="py-20 text-center border-2 border-dashed border-zinc-800 rounded-xl bg-zinc-900/20">
